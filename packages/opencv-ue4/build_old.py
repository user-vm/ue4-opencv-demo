...
if args.android:
    if subprocess.call(["conan", "create", ".", "adamrehn/{}".format(channel), "--profile:build", "ue4", "--profile:host", args.android_conan_profile_host,
                        "--build", "UElibPNG", "--build", "libjpeg", "--build", "zlib", "--build", "opencv-ue4", "--build", "ue4lib"]) != 0:
        sys.exit(1)
else:
